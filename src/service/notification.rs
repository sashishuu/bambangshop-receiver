...
impl NotificationService {
    pub async fn subscribe_request(payload: SubscriberRequest, notification: Notification) -> Result<()> {
...
async fn unsubscribe_request(product_type: String) -> Result<SubscriberRequest> {
    ...
    let request_url: String = format!("{}/notification/unsubscribe/{}/{}", APP_CONFIG.get_publisher_root_url(), product_type_str, notification_receiver_url);
    let request = REQUEST_CLIENT
        .post(request_url.clone())
        .header("Content-type", "application/json")
        .header("Accept", "application/json")
